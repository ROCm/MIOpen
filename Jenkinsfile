        ...
        stage("Full tests I"){
            parallel{
                stage('Int8 Hip Release All') {
                    agent{ label rocmnode("vega20") }
                    steps{
                        buildJob('hcc', '-DMIOPEN_TEST_INT8=On -DBUILD_DEV=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DCMAKE_BUILD_TYPE=release', "", image + "rocm", "/opt/rocm")
                    }
                }
                ...
                stage('Bfloat16 gfx908 Hip Release All Subset') {
                    ...
                    environment{
                        cmd = """
                            ...
                            mkdir build
                            cd build
                            CXX=/opt/rocm/llvm/bin/clang++ cmake -DMIOPEN_USE_COMGR=Off -DBUILD_DEV=On -DCMAKE_BUILD_TYPE=debug -DMIOPEN_GPU_SYNC=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DMIOPEN_TEST_FLAGS="--disable-verification-cache" .. 
                            CTEST_PARALLEL_LEVEL=4 MIOPEN_DEBUG_IMPLICIT_GEMM_NON_XDLOPS_INLINE_ASM=0 MIOPEN_CONV_PRECISE_ROCBLAS_TIMING=0 make -j\$(nproc) check
                        """
        ...
        stage("Full tests II"){
            parallel{
                ...
                stage('Hip Clang Release All') {
                    ...
                    environment{
                        cmd = """
                            ...
                            mkdir build
                            cd build
                            CXX=/opt/rocm/llvm/bin/clang++ cmake -DBUILD_DEV=On -DCMAKE_BUILD_TYPE=release -DMIOPEN_GPU_SYNC=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DMIOPEN_TEST_FLAGS="--disable-verification-cache" .. 
                            CTEST_PARALLEL_LEVEL=4 MIOPEN_DEBUG_IMPLICIT_GEMM_NON_XDLOPS_INLINE_ASM=0 MIOPEN_CONV_PRECISE_ROCBLAS_TIMING=0 make -j\$(nproc) check
                        """
                ...
                stage('FP32 gfx908 Hip Debug All subset') {
                    agent{ label rocmnode("gfx908") }
                    steps{
                        buildJob('hcc', '-DMIOPEN_TEST_GFX908=On -DBUILD_DEV=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DCMAKE_BUILD_TYPE=release', image + "rocm", "/opt/rocm")
                    }
                }
            }
        }
        ...
        stage("Full tests III"){
            parallel{
                ...
                stage('Half Hip Release All') {
                    agent{ label rocmnode("vega20") }
                    steps{
                        buildJob('hcc', '-DMIOPEN_TEST_HALF=On -DBUILD_DEV=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DCMAKE_BUILD_TYPE=release', image + "rocm", "/opt/rocm")
                    }
                }
                ...
                stage('Half gfx908 Hip Release All Subset') {
                    agent{ label rocmnode("gfx908") }
                    steps{
                        buildJob('hcc', '-DMIOPEN_TEST_HALF=On -DMIOPEN_TEST_GFX908=On -DBUILD_DEV=On -DMIOPEN_TEST_ALL=On -DMIOPEN_TEST_LIMIT=4 -DCMAKE_BUILD_TYPE=release', image + "rocm", "/opt/rocm")
                    }
                }
            }
        }
