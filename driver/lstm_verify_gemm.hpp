...
//	memset(wk_state, 0, numlayer * batch_n * hy_stride * sizeof(T));
	...
	for (int li = 0; li < numlayer; li++)
	{
		...
		if (li == 0)
		{
			...
			if (biased)
			{
				for (int bs = 0; bs < batch_n; bs++)
				{
					...
					{
						hid_state[hid_shift + bs * hy_stride + h] += (wei[wei_shift_bias + h] + wei[wei_shift_bias + wei_stride + h]);
					}
				}
			}
		}
		else
		{
			...
			if (biased)
			{
				...
				for (int bs = 0; bs < batch_n; bs++)
				{
					for (int h = 0; h < wei_stride; h++)
					{
						hid_state[hid_shift + bs * hy_stride + h] += (wei[wei_shift_bias_temp + h] + wei[wei_shift_bias_temp + bi * wei_stride + h]);

						if (bidirection)
						{
							hid_state[hid_shift + bs * hy_stride + h] += wei[wei_shift_bias_temp + wei_stride + h];
						}
					}
				}
			}
		...
		for (int ti = 0; ti < seqLength; ti++)
		{
			...
			int wei_shift = li == 0 ? (in_h * wei_stride) : ((in_h + hy_h) * wei_stride + (li - 1) * (bi * hy_h + hy_h) * wei_stride + bi * hy_h * wei_stride);
			...
			for (int bs = 0; bs < in_n[ti]; bs++)
			{
				...
				{
					hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + h], 2) * activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + 3 * hy_h + h], 1);
					if (ti == 0)
					{
						hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + hy_h + h], 2) * cx_state[hx_shift + bs * h_stride + h];
					}
					else
					{
						int prec_shift = li * batch_n * hy_stride + (bacc - in_n[ti - 1]) * hy_stride + bi * 4 * hy_h;

						hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + hy_h + h], 2) * hid_state[prec_shift + bs * hy_stride + h];
					}

					hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h] += activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + 2 * hy_h + h], 2) * activfunc(hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h], 1);

					cy_state[hx_shift + bs * h_stride + h] = hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h];
					hy_state[hx_shift + bs * h_stride + h] = hid_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h];
				}
			}
				...
				for (int bs = 0; bs < in_n[seqLength - 1 - ti]; bs++)
				{
					for (int h = 0; h < hy_h; h++)
					{
						...
						else
						{
							...
							{
								int prec_shift = li * batch_n * hy_stride + (baccbi + in_n[seqLength - 1 - ti]) * hy_stride + bi * 4 * hy_h + hy_h;

								hid_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] += activfunc(hid_state[hid_shift + (baccbi + bs) * hy_stride + 5 * hy_h + h], 2) * hid_state[prec_shift + bs * hy_stride + h];
							}
						}

						hid_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h] += activfunc(hid_state[hid_shift + (baccbi + bs) * hy_stride + 6 * hy_h + h], 2) * activfunc(hid_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h], 1);

						cy_state[hx_shift + bs * h_stride + hy_h + h] = hid_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h];
						hy_state[hx_shift + bs * h_stride + hy_h + h] = hid_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h];
					}
				}
			}
	...
	int wei_shift_bias_temp = wei_shift_bias + 2 * wei_stride + (bi + 1) * (numlayer - 1) * wei_stride;
...
template <typename T>
void RunLSTMBackwardDataGEMMCPUVerify(std::vector<T>& din_host,
	std::vector<T>& wei, // [ input_state_weight_trans
						 // hidden_state_weight0_trans input1_trans
						 // hidden1_trans ... output_weight;
						 // bidirectional reversed weights ]
		...
		for (int ti = seqLength - 1; ti >= 0; ti--)
		{
			...
			if (ti == seqLength - 1)
			{
				for (int bs = 0; bs < in_n[ti]; bs++)
				{
					for (int h = 0; h < hy_h; h++)
					{
						dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h] += dhy_state[hx_shift + bs * h_stride + h];
						dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += dcy_state[hx_shift + bs * h_stride + h];
					}
				}
				...
				if (bidirection)
				{
					for (int bs = 0; bs < in_n[seqLength - 1 - ti]; bs++)
					{
						for (int h = 0; h < hy_h; h++)
						{
							dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h] += dhy_state[hx_shift + bs * h_stride + hy_h + h];
							dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] += dcy_state[hx_shift + bs * h_stride + hy_h + h];
						}
					}
				}
			}
				...
				for (int h = 0; h < hy_h; h++)
				{
					if (ti < seqLength - 1)
					{
						if (bs < in_n[ti + 1])
						{
							...
							dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += dh_state[pretime_shift + bs * hy_stride + bi * 4 * hy_h + h] * activfunc(rsvspace[pretime_shift + bs * hy_stride + hy_h + h], 2);
						}
					}					
					dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h] * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h], 1) * activfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + 2 * hy_h + h], 2);

					if (ti == 0)
					{
						dh_state[hid_shift + (bacc + bs) * hy_stride + hy_h + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] * cx_state[hx_shift + bs * h_stride + h] * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + hy_h + h], 2);
					}
					else
					{
						int pretime_shift = li * batch_n * hy_stride + (bacc - in_n[ti - 1]) * hy_stride;

						dh_state[hid_shift + (bacc + bs) * hy_stride + hy_h + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] * rsvspace[pretime_shift + bs * hy_stride + bi * 4 * hy_h + h] * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + hy_h + h], 2);
					}
					dh_state[hid_shift + (bacc + bs) * hy_stride + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] * activfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + 3 * hy_h + h], 1) * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + h], 2);
					dh_state[hid_shift + (bacc + bs) * hy_stride + 2 * hy_h + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h] * activfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h], 1) * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + 2 * hy_h + h], 2);
					dh_state[hid_shift + (bacc + bs) * hy_stride + 3 * hy_h + h] += dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h] * activfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + h], 2) * dervactivfunc(rsvspace[hid_shift + (bacc + bs) * hy_stride + 3 * hy_h + h], 1);
				
					dcx_state[hx_shift + bs * h_stride + h] = dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 4 * hy_h + h];
					dhx_state[hx_shift + bs * h_stride + h] = dh_state[hid_shift + (bacc + bs) * hy_stride + bi * 5 * hy_h + h];
				}
			}
			...
			if (bidirection)
			{
				for (int bs = 0; bs < in_n[seqLength - 1 - ti]; bs++)
				{
					for (int h = 0; h < hy_h; h++)
					{
						if (ti < seqLength - 1)
						{
							int pretime_shift = li * batch_n * hy_stride + (baccbi - in_n[seqLength - 2 - ti]) * hy_stride;

							dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] += dh_state[pretime_shift + bs * hy_stride + bi * 4 * hy_h + hy_h + h] * activfunc(rsvspace[pretime_shift + bs * hy_stride + 5 * hy_h + h], 2);
						}
						dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h] * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h], 1) * activfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 6 * hy_h + h], 2);

						if (ti == 0)
						{
							dh_state[hid_shift + (baccbi + bs) * hy_stride + 5 * hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] * cx_state[hx_shift + bs * h_stride + hy_h + h] * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 5 * hy_h + h], 2);
						}
						else
						{
							if (bs < in_n[seqLength - ti])
							{
								int pretime_shift = li * batch_n * hy_stride + (baccbi + in_n[seqLength - 1 - ti]) * hy_stride;

								dh_state[hid_shift + (baccbi + bs) * hy_stride + 5 * hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] * rsvspace[pretime_shift + bs * hy_stride + bi * 4 * hy_h + hy_h + h] * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 5 * hy_h + h], 2);
							}
						}
						dh_state[hid_shift + (baccbi + bs) * hy_stride + 4 * hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] * activfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 7 * hy_h + h], 1) * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 4 * hy_h + h], 2);
						dh_state[hid_shift + (baccbi + bs) * hy_stride + 6 * hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h] * activfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h], 1) * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 6 * hy_h + h], 2);
						dh_state[hid_shift + (baccbi + bs) * hy_stride + 7 * hy_h + h] += dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h] * activfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 4 * hy_h + h], 2) * dervactivfunc(rsvspace[hid_shift + (baccbi + bs) * hy_stride + 7 * hy_h + h], 1);
					
						dcx_state[hx_shift + bs * h_stride + hy_h + h] = dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 4 * hy_h + hy_h + h];
						dhx_state[hx_shift + bs * h_stride + hy_h + h] = dh_state[hid_shift + (baccbi + bs) * hy_stride + bi * 5 * hy_h + hy_h + h];
					}
				}
			}
	...
	for (int li = 0; li <= numlayer; li++)
	{
		...
		if (li < numlayer)
		{
			...
			for (int ti = 0; ti < seqLength; ti++)
			{
				...
				{
					pretime_shift = li * batch_n * hy_stride + (bacc - in_n[ti - 1]) * hy_stride + bi * 5 * hy_h;
				...
				if (bidirection)
				{
					...
					{
						pretime_shift = li * batch_n * hy_stride + (bacc + in_n[ti]) * hy_stride + bi * 5 * hy_h;
