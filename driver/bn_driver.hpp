...
class BatchNormDriver : public Driver 
{
    public:
        ...
        void runCPUFwdTrain(double epsilon, double eAF, int batch_sz, int channels, int height, int width);
...
int BatchNormDriver<T>::GetandSetData() {
    ...
    if(bn_mode==miopenBNPerActivation){//1xCxHxW
        sb_len.push_back(1);
        sb_len.push_back(in_len[1]);
        sb_len.push_back(in_len[2]);
        sb_len.push_back(in_len[3]);
    }else if(bn_mode==miopenBNSpatial){//1xCx1x1
        sb_len.push_back(1);
        sb_len.push_back(in_len[1]);
        sb_len.push_back(1);
        sb_len.push_back(1);
...
int BatchNormDriver<T>::AddCmdLineArgs() {
    inflags.AddInputFlag("forw", 'F', "0", "Run Forward Train (off: 0, train: 1, inference: 2) Batch Normalization (Default=0)", "int");
    inflags.AddInputFlag("back", 'b', "0", "Backwards Propagation (off: 0, on: 1) Batch Normalization (Default=0)", "int");
    ...
    inflags.AddInputFlag("mode", 'm', "0", "Normalization Mode (per-activation (0) or spatial (1)) (Default=0)", "int");
    inflags.AddInputFlag("save", 's', "0", "Save off mean and inverse variance, or on backprop, use these values. (Default=0)", "int");
    inflags.AddInputFlag("run", 'r', "0", "Keep running mean and variance, or on inference, use these values. (Default=0)", "int");
...
//keep running mean and variance
    ...
    if(back && forw){
        printf("Warning: Deactivate forward to run backward on Batch Norm.\nRunning forward only.\n");
        back = 0;
    }
...
int BatchNormDriver<T>::createSaveBuffers(){
    ...
    if(status != CL_SUCCESS) printf("Error copying data to GPU\n");
...
int BatchNormDriver<T>::AllocateBuffersAndCopy() {
    ...
    if(status != CL_SUCCESS) printf("Fatal: Error copying data to GPU\nExiting...\n\n");
...
void BatchNormDriver<T>::runGPUFwdInference(double epsilon, T alpha, T beta){
    ...
    if(keepRunningMeanVar){ //use precalculated mean and variance
            miopenBatchNormalizationForwardInference(
                                    ...
                                    biasScaleTensor, scale_dev->GetMem(), bias_dev->GetMem(),
                                    runningMean_dev->GetMem(),
                                    runningVariance_dev->GetMem(),
                                    epsilon);
    }else{ //recalculate mean and variance
            miopenBatchNormalizationForwardInference(
                                    GetHandle(),
                                    bn_mode,
                                    &alpha,	
...
int BatchNormDriver<T>::RunForwardGPU() {
    ...
    //if run fwd train
    if(forw == 1){//training only
            runGPUFwdTrain(epsilon, eAF, alpha, beta);
    }else if(forw == 2){//inference only
...
template<typename T>
void BatchNormDriver<T>::runCPUFwdInference(double epsilon, int batch_sz, int channels, int height, int width){

    if(bn_mode==miopenBNPerActivation){//1xCxHxW
        miopenBNFwdInferPerActivationRunHost(/* alpha, beta, */batch_sz, channels, height, width,
                        in.data(), out_host.data(), scale_host.data(), bias_host.data(), 
                        epsilon, keepRunningMeanVar, runningMean_host.data(), runningVariance_host.data());
    }else if(bn_mode==miopenBNSpatial){//1xCx1x1
        miopenBNFwdInferSpatialRunHost(/* alpha, beta, */ batch_sz, channels, height, width,
                        in.data(), out_host.data(), scale_host.data(), bias_host.data(),
                        epsilon, keepRunningMeanVar, runningMean_host.data(), runningVariance_host.data());
    }else{
...
template<typename T>
void BatchNormDriver<T>::runCPUFwdTrain(double epsilon, double eAF, int batch_sz, int channels, int height, int width){

    if(bn_mode==miopenBNPerActivation){//1xCxHxW
        miopenBNFwdTrainPerActivationRunHost(/* alpha, beta, */ batch_sz, channels, height, width,
                        in.data(), out_host.data(), scale_host.data(), bias_host.data(), 
                        epsilon, saveMeanVar, keepRunningMeanVar, 
                        saveMean_host.data(),saveInvVariance_host.data(),
                        runningMean_host.data(), runningVariance_host.data(), eAF);
    }else if(bn_mode==miopenBNSpatial){//1xCx1x1
        miopenBNFwdTrainSpatialRunHost(/* alpha, beta, */ batch_sz, channels, height, width,
                        in.data(), out_host.data(), scale_host.data(), bias_host.data(),
                        epsilon, saveMeanVar, keepRunningMeanVar,
                        saveMean_host.data(),saveInvVariance_host.data(),
                        runningMean_host.data(), runningVariance_host.data(), eAF);
...
int BatchNormDriver<T>::RunBackwardGPU() {
    ...
    if(saveMeanVar){
        miopenBatchNormalizationBackward(
                            ...
                            inputTensor, in_dev->GetMem(), 
                            dyInputTensor, dyin_dev->GetMem(),
                            dxOutputTensor, dxout_dev->GetMem(),
                            biasScaleTensor, scale_dev->GetMem(), 
                            dscale_dev->GetMem(), dbias_dev->GetMem(),
                            epsilon,
                            saveMean_dev->GetMem(),
                            saveInvVariance_dev->GetMem());
    }else{
...
//jump out since we are forcing forward off when doing backwards.
    if(!forw) return miopenStatusSuccess;
    ...
    if(forw == 1){
        ...
        if(keepRunningMeanVar){//copy back for verification
            ...
            if (errorRunMean > maxrms  || std::isnan(errorRunMean)){
                std::cout<<"Forward train batch norm verification failed on running mean: " << errorRunMean <<"\n";
                anError = true;
                #if (MIO_BN_DEBUG==1)
                for(int i = 0; i<runningMean.size() && i<runningMean_host.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
                    ...
                    if(diff > tolerance){
                        ...
                        std::cout << ", diff[" << i << "]: " << float(fabs(runningMean[i]) - fabs(runningMean_host[i])) << std::endl;
                    }
                }
                #endif
            }else{
            ...
            if (errorRunVar > maxrms  || std::isnan(errorRunVar)){
                    std::cout<<"Forward train batch norm verification failed on running variance: " << errorRunVar <<"\n";
                    anError = true;
                    #if (MIO_BN_DEBUG==1)
                    for(int i = 0; i<runningVariance.size() && i<runningVariance_host.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
                            ...
                            if(diff > tolerance){
                                ...
                                std::cout << ", diff[" << i << "]: " << float(fabs(runningVariance[i]) - fabs(runningVariance_host[i])) << std::endl;
                            }
                    }
                    #endif
            }else{
        ...
        if(saveMeanVar){//copy back for verification
            ...
            if (errorSaveMean > maxrms  || std::isnan(errorSaveMean)){
                std::cout<<"Forward train batch norm verification failed on saved mean: " << errorSaveMean <<"\n";
                anError = true;
                #if (MIO_BN_DEBUG==1)
                for(int i = 0; i<saveMean.size() && i<saveMean_host.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
                    ...
                    if(diff > tolerance){
                        ...
                        std::cout << ", diff[" << i << "]: " << float(fabs(saveMean[i]) - fabs(saveMean_host[i])) << std::endl;
                    }
                }
                #endif
            ...
            if (errorSaveVar > maxrms  || std::isnan(errorSaveVar)){
                std::cout<<"Forward train batch norm verification failed on saved inverse variance: " << errorSaveVar <<"\n";
                anError = true;
                #if (MIO_BN_DEBUG==1)
                for(int i = 0; i<saveInvVariance.size() && i<saveInvVariance_host.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
                    ...
                    if(diff > tolerance){
                        ...
                        std::cout << ", diff[" << i << "]: " << float(fabs(saveInvVariance[i]) - fabs(saveInvVariance_host[i])) << std::endl;
                    }
                }
                #endif
            }else{
                std::cout<<"Forward train batch norm verification passed on saved inverse variance.\n";
            }
        }//end if(saveMeanVar)
...
//	T alphaParam = 1, betaParam = 0;
    ...
    if(bn_mode==miopenBNPerActivation){//1xCxHxW
        miopenBNBwdPerActivationRunHost(/* alphaDiff, betaDiff, alphaParam, betaParam, */
                                        batch_sz, channels, height, width,
                                        in.data(), dyin.data(), dxout_host.data(), scale.data(),
                                        dscale_host.data(), dbias_host.data(),
                                        epsilon, saveMeanVar, 
                                        saveMean_host.data(), saveInvVariance_host.data());
    }else if(bn_mode==miopenBNSpatial){//1xCx1x1
   ...
   if (errordxout > maxrms  || std::isnan(errordxout)){
        ...
        for(int i = 0; i<dxout.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
            ...
            if(diff > tolerance){
                ...
                std::cout << "\tratioH: " << fabs(fabs(dxout[i]) - fabs(dxout_host[i]))/fabs(dxout_host[i]) << std::endl;
            }
        }
        #endif
    ...
    if (errordscale > maxrms  || std::isnan(errordscale)){
        ...
        for(int i = 0; i<dscale.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
            ...
            if(diff > tolerance){
                ...
                std::cout << "\tdiff[" << i << "]: " << float(fabs(dscale[i]) - fabs(dscale_host[i]));
                std::cout << "\tratioH: " << fabs(fabs(dscale[i]) - fabs(dscale_host[i]))/fabs(dscale_host[i]) << std::endl;
            }
        }
        #endif
    ...
    if (errordbias > maxrms  || std::isnan(errordbias)){
        ...
        for(int i = 0; i<dbias.size() && i < MIO_BN_MAX_DEBUGLOOP;i++){
            ...
            if(diff > tolerance){
                ...
                std::cout << "\tratioH: " << fabs(fabs(dbias[i]) - fabs(dbias_host[i]))/fabs(dbias_host[i]) << std::endl;
            }
        }
        #endif
    }else{
    ...
    if(!anError) std::cout << "Backwards prop batch norm verified on CPU and GPU." << std::endl;
