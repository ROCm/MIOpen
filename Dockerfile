...
RUN wget https://repo.radeon.com/amdgpu-install/6.1.2/ubuntu/focal/amdgpu-install_6.1.60102-1_all.deb --no-check-certificate
RUN apt-get update && \
DEBIAN_FRONTEND=noninteractive apt-get install -y --allow-unauthenticated \
    ./amdgpu-install_6.1.60102-1_all.deb

# Add rocm repository
RUN export ROCM_APT_VER=6.1.2;\
echo $ROCM_APT_VER &&\
