...
RUN wget https://repo.radeon.com/amdgpu-install/5.5.1/ubuntu/focal/amdgpu-install_5.5.50501-1_all.deb  --no-check-certificate
RUN apt-get update && \
DEBIAN_FRONTEND=noninteractive apt-get install -y --allow-unauthenticated \
    ./amdgpu-install_5.5.50501-1_all.deb

# Add rocm repository
RUN export ROCM_APT_VER=5.5.1;\
echo $ROCM_APT_VER &&\
