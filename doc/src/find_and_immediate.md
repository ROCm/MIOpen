...
- `OPTIMIZED_HYBRID`, or `4`: Optimized hybrid Find: Checks the [Find-Db](https://rocmsoftwareplatform.github.io/MIOpen/doc/html/finddb.html) for an entry. If there is a Find-Db hit, use that entry. If there is a miss, use the existing Find machinery but skip kernels with slow compilation. Faster start-up times than Hybrid Find, may have GPU performance drop.
