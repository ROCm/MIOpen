...
- `NT_HYBRID`, or `5`: NT Hybrid Find (EXPERIMENTAL): It works very like Fast Hybrid mode, but in case of Find-db miss, skips _untuned_ tunable kernels (instead of slow-compiling ones). We expect faster start-up times than Fast Hybrid Find, and better GPU performance. Use with caution, this mode is experimental for now.
