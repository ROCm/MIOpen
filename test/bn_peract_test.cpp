...
struct verify_forward_train_bn_per_activation
{
            ...
            for (int row = 0; row < height; row++){ //via rows
                for(int column = 0; column < width; column++){// via columns
                    ...
                    for (int bidx = 0; bidx < n_batch; bidx++){ //via mini_batch
                        // #1 calculate the mean :: iterating through the stack of images in the mini_batch
                        mean_accum += input(bidx,cidx,row,column);
                    }	
                    mean_accum/=n;

                    elemStd = variance_accum = 0.;
                    // #2 calculate the variances :: sigma^2 = (1/batch_mean) * sum( (x_i - batch_mean)^2 )
                    for (int bidx = 0; bidx < n_batch; bidx++){ //via mini_batch
                        elemStd = (input(bidx,cidx,row,column) - mean_accum);// (x_i - mean) //this is reused but needs recalc
                    ...
                    for (int bidx = 0; bidx < n_batch; bidx++){ //via mini_batch
                        ...
                        out(bidx,cidx,row,column) = scale(0,cidx,row,column)*inhat + shift(0,cidx,row,column);
                    }//end for(n_batch)  
                    
                    newRunMean = runMean(0,cidx,row,column)*(1.0-expAvgFactor);
                    runMean(0,cidx,row,column) = mean_accum*expAvgFactor+newRunMean;//newMean*factor + tmp

                    // var(n+1) = p * var(n-1) + (1 - p)*(b/b-1)*var(n)
                    adjust = (n_batch == 1) ? variance_accum : (n/(n-1.0))*variance_accum;
                    runVar(0,cidx,row,column) = expAvgFactor*runVar(0,cidx,row,column) + (1 - expAvgFactor)*adjust;
    ...
    std::tuple<tensor<T>,tensor<T>,tensor<T>,tensor<T>,tensor<T>> gpu() {
        ...
        miopen::BatchNormForwardTraining(handle, miopenBNPerActivation, &alpha, &beta, input.desc, in_dev.get(), out.desc, out_dev.get(), 
                                            scale.desc, scale_dev.get(), shift_dev.get(), expAvgFactor, 
                                            runMean_dev.get(), runVar_dev.get(), epsilon, saveMean_dev.get(), saveInvVar_dev.get());
...
struct verify_forward_infer_bn_per_activation_recalc
{
    ...
    tensor<T> gpu() {
        ...
        miopen::BatchNormForwardInference(handle, miopenBNPerActivation, &alpha, &beta, 
...
struct verify_forward_infer_bn_per_activation_use_est
{
    ...
    void fail(int )
    {
        std::cout << "Forward Inference Per Activation Batch Normalization Use Estimated: " << std::endl;
        std::cout << "Input tensor: " << input.desc.ToString() << std::endl;
    }
};
...
struct verify_backward_bn_per_activation_use_saved
{
    ...
    std::tuple<tensor<T>,tensor<T>,tensor<T>> gpu(){
        ...
        miopen::BatchNormBackward(handle, miopenBNPerActivation, &alpha, &beta, &alpha, &beta,
                                        x_input.desc, xin_dev.get(), dy_input.desc, dyin_dev.get(), dx_out.desc, dx_out_dev.get(), 
                                        scale.desc, scale_dev.get(), dscale_dev.get(), dshift_dev.get(), epsilon, 
                                        savedMean_dev.get(), savedInvVar_dev.get());
    ...
    void fail(int badtensor){
        std::cout << "Backward Batch Per Activation Normalization Using Saved Mean and Variance: " << std::endl;
...
struct verify_backward_bn_per_activation_recalc
{
    ...
    void fail(int badtensor){
        std::cout << "Backward Batch Per Activation Normalization Recalc Mean and Variance: " << std::endl;
        std::cout << "X Input tensor: " << x_input.desc.ToString() << std::endl;
        std::cout << "Delta Y Input tensor: " << dy_input.desc.ToString() << std::endl;
        switch (badtensor){
            case(0): 
...
struct batch_norm_per_activation_driver : test_driver
{
	...
	if(n==1){//Invalid batch size for batch norm tests.
		return;
	}
        ...
        verify(verify_forward_infer_bn_per_activation_use_est<T>{ input, scale, shift, estMean, estVar });
       
        //backprop recalc
        auto dy_input = tensor<T>{n,c,h,w}.generate(rand_gen{});//= std::get<0>(outpair.first);//
        ...
        verify(verify_backward_bn_per_activation_use_saved<T>{ input, dy_input, scale, savedMean, savedInvVar });
    }
};
