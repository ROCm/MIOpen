...
struct SpeedTestDriver : public test_driver
{
    ...
    void run()
    {
        ...
        case Sequences::Unknown: std::cerr << "Unknown sequence." << std::endl; std::exit(-1); // NOLINT (concurrency-mt-unsafe)
        }
    }
    ...
    void RunCore() const
    {
        ...
        case Modes::Unknown: std::cerr << "Unknown mode." << std::endl; std::exit(-1); // NOLINT (concurrency-mt-unsafe)
        }
    }
    ...
    void Test(const TRuleGetter& rule_getter) const
    {
        ...
        case Instances::Static:
            ...
            break;
        case Instances::Unknown: std::cerr << "Unknown instance type" << std::endl; std::exit(-1); // NOLINT (concurrency-mt-unsafe)
        }
    }
    ...
    void Standard() const
    {
        ...
        case Sequences::IRange: std::cerr << "irange is only for boost" << std::endl; std::exit(-1); // NOLINT (concurrency-mt-unsafe)
        }
    }
