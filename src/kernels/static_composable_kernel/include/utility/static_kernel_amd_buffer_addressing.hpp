...
make_raw_buffer_resourse(T* p_src_block, short stride, int num, int flags)
{
#if CK_USE_AMD_BUFFER_PTR_TYPE
    // void *p, short stride, int num, int flags
    //  ret __amdgpu_buffer_rsrc_t
    buffer_resourse_t src_block_config =
        __builtin_amdgcn_make_buffer_rsrc(p_src_block, stride, num, flags);
#else
