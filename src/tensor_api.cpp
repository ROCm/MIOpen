...
mlopenStatus_t mlopenTransformTensor(mlopenHandle_t handle,
	...
	return mlopen::try_([&] {
		return mlopen::deref(yDesc).TransformTensor(handle, 
				alpha,
				mlopen::deref(xDesc),
				DataCast(x),
				beta,
...
mlopenStatus_t mlopenOpTensor(mlopenHandle_t handle,
	...
	return mlopen::try_([&] {
		return mlopen::deref(cDesc).OpTensor(handle,
				tensorOp,
				alpha1,
				mlopen::deref(aDesc),
				DataCast(A),
...
mlopenStatus_t mlopenSetTensor(mlopenHandle_t handle,
	...
	return mlopen::try_([&] {
		return mlopen::deref(yDesc).SetTensor(handle,
				DataCast(y),
				valuePtr);
	});
...
mlopenStatus_t mlopenScaleTensor(mlopenHandle_t handle,
	...
	return mlopen::try_([&] {
		return mlopen::deref(yDesc).ScaleTensor(handle,
				DataCast(y),
				alpha);
	});
