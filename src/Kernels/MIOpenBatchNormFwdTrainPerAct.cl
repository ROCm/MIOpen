...
*/
                ...
                pvt_newRunMean = mad((_FLOAT) -expAvgFactor,pvt_runMean, pvt_runMean);//tmp = oldRunMean*(1-factor)
                resultRunningMean[adjIndex] = mad((_FLOAT)mean_accum,(_FLOAT) expAvgFactor, pvt_newRunMean);//newMean*factor + tmp
            #endif
            ...
            for(unsigned int n = 0; n < MIO_BN_N; n++){
                ...
                variance_accum = mad(elemStd, elemStd, (_FLOAT) variance_accum); // sum{ (x_i - mean)^2 }
            }//end for(n)
            variance_accum /= N; // (1/N)*sum{ (x_i - mean)^2 }
                ...
                resultRunningVariance[adjIndex] = mad((_FLOAT)expAvgFactor,resultRunningVariance[adjIndex], tmp);
            #endif
