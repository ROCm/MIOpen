...
__kernel void BatchNormFwdTrainSpatialSingleVecNorm(
    ...
    unsigned int coff = (xgid+coffset);
    unsigned int cidx = coff*MIO_BN_HW;
    ...
    if(ygid == 0){
        //Save mean and calculate and save running mean
        #if (MIO_SAVE_MEAN_VARIANCE == 1)
            resultSaveMean[coff] = mean;
            resultSaveInvVariance[coff] = invVariance;
        #endif

        #if (MIO_RUNNING_RESULT == 1)
            _FLOAT pvt_runMean = resultRunningMean[coff];
            _FLOAT pvt_newRunMean = mad((_FLOAT)-expAvgFactor,pvt_runMean, pvt_runMean);//tmp = oldRunMean*(1-factor)
            resultRunningMean[coff] = mad(mean, (_FLOAT)expAvgFactor, pvt_newRunMean);//newMean*factor + tmp
            ...
            resultRunningVariance[coff] = mad((_FLOAT)expAvgFactor,resultRunningVariance[coff], tmp);
        #endif    
    }
    #endif
}//end spatial norm
