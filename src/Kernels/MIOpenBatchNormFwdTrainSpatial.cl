...
__kernel void BatchNormFwdTrainSpatialSingleNorm(
    ...
    if(ygid<MIO_BN_HW){
        ...
        for(int n = 0; n < MIO_BN_N; n++){
            ...
            if(index >= MIO_BN_CHW*MIO_BN_N) printf("index in single norm for mean is %d > || = %d\n", index, MIO_BN_CHW*MIO_BN_N);//TODO: DLOWELL debug
            else mean += in[index];
        }  
    }
    lcl_data[ylid] = mean;
    ...
    if(ygid<MIO_BN_HW){
        ...
        for(int n = 0; n < MIO_BN_N; n++){
            index = n*MIO_BN_CHW + cidx + ygid;
            if(index >= MIO_BN_CHW*MIO_BN_N) printf("index in single norm for variance is %d > || = %d\n", index, MIO_BN_CHW*MIO_BN_N);//TODO: DLOWELL debug
            else{
    ...
    if(ygid<MIO_BN_HW){
        ...
        for(unsigned int n = 0; n < MIO_BN_N; n++){//apply normalization
            ...
            if(index >= MIO_BN_CHW*MIO_BN_N) printf("index is %d > || = %d in single norm\n", index, MIO_BN_CHW*MIO_BN_N);
            else{
    ...
    if(ygid == 0){
        if(xgid >= MIO_BN_C) printf("xgid is %d > || = %d in single norm\n", xgid, MIO_BN_C);
        else{
