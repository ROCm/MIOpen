...
#define NUM_IM_BLKS_Y 1

	local float local_im[324];

	int witem_ch = lid / THREADS_PER_CH;
	int wg_ch = gid / (NUM_IM_BLKS_X * NUM_IM_BLKS_Y);

	int im_x = ((gid % (NUM_IM_BLKS_X*NUM_IM_BLKS_Y)) % NUM_IM_BLKS_X) * 16;
	int im_y = ((gid % (NUM_IM_BLKS_X*NUM_IM_BLKS_Y)) / NUM_IM_BLKS_X) * 16;
	
	int out_cols_wg = im_x + 16 <= out_w ? 16 : out_w - im_x;
	int out_rows_wg = im_y + 16 <= out_h ? 16 : out_h - im_y;

	int im_cols_wg = out_cols_wg + 2*pad_w;
	int inner_lid = lid;
	int row_to_use = inner_lid / 18;
	int col_to_use = inner_lid % 18;
	int lm_offset = row_to_use*18 + col_to_use;
	...
	if(lid < out_cols_wg * out_rows_wg) {
		...
		int col_y = (gid/(NUM_IM_BLKS_X*NUM_IM_BLKS_Y)) * out_h * out_w * wei_h * wei_w;
