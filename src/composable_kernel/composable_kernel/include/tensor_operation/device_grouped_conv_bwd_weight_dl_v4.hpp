...
__device__ T warp_shuffle_down(const T& v_local, uint32_t lane_delta)
{
    ...
    const int32_t v_remote_tmp = __builtin_amdgcn_ds_bpermute(
        (__lane_id() << 2) + (lane_delta << 2), __builtin_bit_cast(int32_t, v_local));

    return __builtin_bit_cast(T, v_remote_tmp);
#endif
}
template <typename T>
...
template <>
void __device__ global_atomic_add<ushort2_t>(ushort2_t* p, ushort2_t v)
{
#if defined(__gfx942__) || defined(__gfx950__)
...
__device__ void block_sync_lds_new()
{
...
template <index_t BlockSize,
        ...
        //   typename FilterParam,   // tuple<dilation, stride, padding>
          typename FilterDilation,
          typename FilterStride,
          typename FilterPadding,
...
struct GridwiseGroupedConv2DBwdWeightDlV4
{
    ...
    static constexpr index_t Pad_H = FilterPadding{}.At(I0);
    static constexpr index_t Pad_W = FilterPadding{}.At(I1);

    static constexpr index_t Stride_H = FilterStride{}.At(I0);
    static constexpr index_t Stride_W = FilterStride{}.At(I1);

    // Only support dilation = 1 for now
    static constexpr index_t Dilation_Y = FilterDilation{}.At(I0);
    static constexpr index_t Dilation_X = FilterDilation{}.At(I1);
...
//
        ...
        else
        {
            ...
            block_sync_lds_new();
            p_share_acc[threadIdx.x] = __builtin_bit_cast(uint32_t, acc);
            block_sync_lds_new();
    ...
    struct Argument
    {
        ...
        Argument(const InDataType* p_in_grid,
                 ...
                 const Array<index_t, NDimSpatial + 3>& in_g_n_c_wis_lengths, // input
                 const Array<index_t, NDimSpatial + 3>& in_g_n_c_wis_strides,
                 const Array<index_t, NDimSpatial + 3>& wei_g_k_c_xs_lengths, // weight
                 const Array<index_t, NDimSpatial + 3>& wei_g_k_c_xs_strides,
                 const Array<index_t, NDimSpatial + 3>& out_g_n_k_wos_lengths, // output
                 const Array<index_t, NDimSpatial + 3>& out_g_n_k_wos_strides,
                 index_t k_batch)
        ...
        Array<index_t, NDimSpatial + 3> in_g_n_c_wis_lengths_;
        Array<index_t, NDimSpatial + 3> in_g_n_c_wis_strides_;
        Array<index_t, NDimSpatial + 3> wei_g_k_c_xs_lengths_;
        Array<index_t, NDimSpatial + 3> wei_g_k_c_xs_strides_;
        Array<index_t, NDimSpatial + 3> out_g_n_k_wos_lengths_;
        Array<index_t, NDimSpatial + 3> out_g_n_k_wos_strides_;
        bool enable_k_split_;
    };

template <index_t MinimumOccupancy = 1>
__device__ static void
    RunWrapper(const InDataType* p_in_grid,
                WeiDataType* p_wei_grid,
                const OutDataType* p_out_grid,
                AccDataType* p_acc_grid,
                const Array<index_t, NDimSpatial + 3>& in_g_n_c_wis_lengths, // input
    ...
    {
        // __shared__ char
        //     p_share_in[ShareMemInSize * NumTilePerBlock];
        // __shared__ char p_share_out[ShareMemOutSize * NumTilePerBlock];
