...
template<int N>
auto GetDeviceInfo(cl_device_id device) MIOPEN_RETURNS(detail::GetDeviceInfoImpl<detail::DeviceAttributeReturnType<N>>::apply(N, device));

template<int N>
auto GetPlatformInfo(cl_platform_id platform) MIOPEN_RETURNS(detail::GetPlatformInfoImpl<detail::PlatformAttributeReturnType<N>>::apply(N, platform));
