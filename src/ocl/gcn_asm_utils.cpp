...
private:
    ...
    {
        static const auto path = std::getenv("TMPDIR");
        if (path != nullptr) {
            return path;
        }
#if defined(P_tmpdir)
...
public:
    ...
    void Open()
    {
        if (pipe(_sides)) { MIOPEN_THROW("Error: pipe()"); }
...
int ExecuteGcnAssembler(const std::string& p, std::vector<std::string>& args, std::istream* in, std::ostream* out)
{
    ...
    if (pid == 0) {
        ...
        if (pid == -1) { MIOPEN_THROW("Error X-AMDGCN-ASM: fork()"); }
        ...
        if (waitpid(pid, &wstatus, 0) != pid) { MIOPEN_THROW("Error: X-AMDGCN-ASM: waitpid()"); }
    }
