                                     ...
                                     const ExaustiveSearchResult& exaustive_search_result) const
{
    ...
    if ((params.n_outputs / 16) * 16 == params.n_outputs && (params.n_inputs / 4) * 4 == params.n_inputs)
    {
        ...
        else
        {
            ...
            kernel.comp_options =
                ...
                std::string(" -DMLO_IN_BATCH_STRIDE=") + std::to_string(params.in_batch_stride) +
                std::string(" -DMLO_IN_CHANNEL_STRIDE=") + std::to_string(params.in_channel_stride) +
                std::string(" -DMLO_IN_STRIDE=") + std::to_string(params.in_stride) +
            ...
            kernel.kernel_name =
                (params.pad0 == 0 && params.kernel_stride0 == 1) ? "MIOpenConv1x1" : "MIOpenConv1x1pquv";
        }
    }
    else
    {
        ...
        int C1x1_PIXLEFT = (DIVBY4 == 1) ? 0 : params.in_width * params.in_height - (MAP_SZ4 - 1) * read_unit;
        ...
        kernel.comp_options =
            ...
            std::string(" -DMLO_N_OUTPUTS=") + std::to_string(static_cast<long long>(params.n_outputs)) +
            std::string(" -DMLO_N_INPUTS=") + std::to_string(static_cast<long long>(params.n_inputs)) +
            ...
            + std::string(" -DMLO_N_LCL_OUT_MAPS=") +
            std::to_string(
                static_cast<long long>(result.n_out_pix_tiles)) // # output pixel tiles per wk-item (ALU)
