...
# Define the process_single_config function
process_single_config() {
    local config_name=$1
    local kernel_times_path=$2
    ...
    local log_path_config="../logs/${mode}_${algs}_${config_name}${tuning}${pid}.log"

    # Time the MIOpen driver call and log the output to the log file.
    echo "Log file: $log_path_config"
    mkdir -p $(dirname "$log_path_config")

    echo "=== Execution started at $(date) ===" > "$log_path_config"
    echo "" >> "$log_path_config"
    {
        time ( 
            ...
            if [ $? -ne 0 ]; then
                echo "ERROR: MIOpenDriver command failed." >> "$log_path_config"
            fi
            echo ""
            echo "=== MIOpenDriver execution time ===" >> "$log_path_config"
        ) 
    } >> "$log_path_config" 2>&1
    echo "" >> "$log_path_config"

    # Record the environment variables and parameters used in the execution
    echo "=== Environment Variables ===" >> "$log_path_config"
    env >> "$log_path_config"
    echo "=== Execution completed at $(date) ===" >> "$log_path_config"

    # Extract kernel times
    extract_kernel_times "$log_path_config" "$kernel_times_path" "$config_name"
    echo "Kernel times extracted to: $kernel_times_path"
}

if [[ "$*" == *"--config"* ]]; then

    configs_raw=$(echo "$*" | sed -n 's/.*--config\s\+\([^-]*\).*/\1/p' | xargs)
    config_list=$(echo "$configs_raw" | tr -s ' ' | sed 's/ *, */,/g')
    ...
    for config_name in "${config_array[@]}"; do
        ...
        # Validate the configuration exists in config file
        if ! jq -e ".$config_name" "$config_file" > /dev/null; then
            echo "Error: Configuration '$config_name' not found in $config_file"
